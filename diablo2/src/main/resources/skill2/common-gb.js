...
new function(){
	...
	i=i==-1?location.href.length:i;
	location.base=location.href.substring(0,i);
	location.arg=[];
	var a=location.href.substring(i+1).split("&");
	for(i=0;i<a.length;++i){
		var p=a[i].split("=");
		location.arg[unescape(p[0])]=unescape(p[1])
	}
}()
/*******************************************************************************
...
function add(b,i){
	return function(){
		var t=parseInt(tabs[b.tab].total.innerHTML)-parseInt(b.value);
		b.value=parseInt(b.value)+i;
		validate(b)();
		tabs[b.tab].total.innerHTML=t+parseInt(b.value);
		sumall();
		return false
	}
}
...
function recount(b){
	return function(){
		validate(b)();
		var v=0;
		for(var j=0;j<tabs[b.tab].box.length;++j)
			v+=parseInt(tabs[b.tab].box[j].value);
		tabs[b.tab].total.innerHTML=v;
		sumall();
		return false
	}
}
function clamp(l,h,n){
	if(isNaN(n))n=0;
	return n<l?l:n>h?h:n
}
function validate(b){
	return function(){
		b.value=clamp(0,999,parseInt(b.value));
		b.blur();
		update();
		return false
	}
...
function sumall(){
	var v=0;
	for(var i=1;i<4;++i)
		v+=parseInt(tabs[i].total.innerHTML);
	tabs.total.innerHTML=v;
	return false
}
function setcurrent(j){
	return function(){
		tabs[tabs.active].active=j;
		update();
		return false
	}
}
...
function showTab(n){
	if(n!=tabs.active){
		tabs[n].tree.style.display="block";
		tabs[tabs.active].tree.style.display="none";
		tabs.active=n;update()
	}
}
function update(){
	tabs.data.innerHTML=display(tabs[tabs.active][tabs[tabs.active].active])
}
/*******************************************************************************
 * Initialize the data
 ******************************************************************************/
...
tabs.write=function(){
	...
	for(var j=0; j<this[i].length; ++j){
		for(var p in s)
			skills[this[i][j]][s[p]]=skills[this[i][j]][skills[s[p]]]
...
tbl.StrSklTree4="技能"
	tbl.plus="+"
		tbl.to="点"
			tbl.partycharama="亚马逊"
				tbl.partycharass="刺客"
					tbl.partycharbar="野蛮人"
						tbl.partychardru="德鲁伊"
							tbl.partycharnec="死灵法师"
								tbl.partycharpal="圣骑士"
									tbl.partycharsor="法师"
										/*******************************************************************************
